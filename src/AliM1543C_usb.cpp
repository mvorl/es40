                               ...
                               int pcidev) : CPCIDevice(cfg, c, pcibus, pcidev)
{
  ...
  printf("%s: $Id: AliM1543C_usb.cpp,v 1.6 2008/03/14 15:30:50 iamcamiel Exp $\n",
         devid_string);
}
...
u32 CAliM1543C_usb::ReadMem_Bar(int func, int bar, u32 address, int dsize)
{
  ...
  switch(bar)
  {
  case 0:   data = usb_hci_read(address, dsize); break;
  default:  printf("%%USB-W-READBAR: Bad BAR %d selected.\n", bar);
  }
                                  ...
                                  u32 data)
{
  switch(bar)
  {
  case 0:   usb_hci_write(address, dsize, data); break;
  default:  printf("%%USB-W-WRITEBAR: Bad BAR %d selected.\n", bar);
  }
